...
foreach $name (sort {$puzzle_number{$a} <=> $puzzle_number{$b}} 
    ...
    printf("%d\t%d\t%d\t%d\t%d\t\%s\n", 
	   ...
	   $puzzle_p_points{$name}/2,
	   $puzzle_p_ops{$name},
	   $name);
}

$tpoints = $total_c_points + $total_p_points/2;
$trating = $total_c_rating + $total_p_rating/2;

$tpp = $total_p_points/2;
$tpr = ($total_p_rating/2);

print "\nScore = $tpoints/$trating [$total_c_points/$total_c_rating Corr + $tpp/$tpr Perf] ($tops total operators)\n";
